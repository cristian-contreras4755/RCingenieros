...
{
    public class VentaPetroamericaAccesoDatos
    {
        #region Venta
        ...
        {
            var agenciaListaInfo = new List<AgenciaPetroamericaInfo>();
            using (var oConexion = new SqlConnection(HelperAccesoDatos.GetCadenaConexion()))
            {
                var sqlComando = new SqlCommand();
                sqlComando.Connection = oConexion;
                sqlComando.CommandText = "ObtenerAgenciaPetroamerica";
                sqlComando.CommandType = CommandType.StoredProcedure;
                sqlComando.Parameters.Add("AgenciaPetroamerica", SqlDbType.VarChar).Value = agenciaPetroamerica;
                ...
                using (SqlDataReader dr = sqlComando.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        agenciaListaInfo.Add(CargarAgenciaPetroamericaInfo(dr));
                    }
                }
                oConexion.Close();
            }
        ...
        private static AgenciaPetroamericaInfo CargarAgenciaPetroamericaInfo(IDataReader dr)
        {
            int colCount = dr.FieldCount;
            var values = new object[colCount];
            ...
            var agenciaInfo = new AgenciaPetroamericaInfo();
            dr.GetValues(values);

            #region Campos
            agenciaInfo.Agencia = Convert.ToString(values[indAgencia]);
            #endregion
